...
func NewRouter(db *pgxpool.Pool, devMode bool) http.Handler {
	...
	r.Group(func(r chi.Router) {
		...
		r.Route("/api/projects/{projectID}/env", func(r chi.Router) {
			r.Get("/", env.list)
			r.Post("/", env.create)
			r.Delete("/{varID}", env.delete)
			r.Get("/{varID}/reveal", env.reveal)
		})
		...
		oasImport := &openAPIImportHandler{db: db}
		r.Post("/api/projects/{projectID}/import/openapi", oasImport.preview)
